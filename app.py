...
def create():
    if request.method == 'POST':    # if submit button was pressed
        ...
        # return render_template('create.html', values=get_values(), exp_name=get_exp_name(), round_num=get_round_num(), src=calculate_filters())    # displays the html file
...
def determine(entries):
    print("determine")
    if 'exp_name' and 'round_num' in entries:
        print("is exp or round")
        exp_name = request.form['exp_name']
        round_num = request.form['round_num']
        remove_add(entries, exp_name, round_num)
    else:
        print("not exp or round")
        exp_name = get_exp_name()
        round_num = get_round_num()
        remove_add(entries, exp_name, round_num)

def remove_add(entries, exp_name, round_num):
    print("remove_add")
    for entry in entries:
        ...
        if len(value_content) > 0:  # if the field has a value, then remove the existing value and add the new one
            ...
            for value in existing_values:
                ...
                db.session.commit()
            # add new value
            new_value = Todo(exp_name=exp_name, round_num=round_num, field=entry, content=value_content)
            try:
